...
new class extends Component {
    ...
    public function payments()
    {
        return Payment::query()
            ...
            ->with(['library', 'membership.seat.room'])
            ...
            ->when($this->search, function ($query) {
                ...
                $query->where(function ($nestedQuery) use ($search) {
                    $nestedQuery
                        ...
                        ->orWhereHas('membership.seat', fn ($seatQuery) => $seatQuery
                            ->where('seat_number', 'like', "%{$search}%"))
                        ->orWhereHas('membership.seat.room', fn ($roomQuery) => $roomQuery
                            ->where('name', 'like', "%{$search}%"));
                });
            })
...
<section class="space-y-8">
    <div class="relative mb-6 w-full">
        <div>
            ...
            <flux:subheading size="lg" class="mb-6">{{ __('Review your cash and online membership payments with verified status details') }}</flux:subheading>
        </div>
    ...
    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900">
            <flux:text class="text-sm text-zinc-500">{{ __('Online Paid') }}</flux:text>
            <flux:heading size="lg" class="mt-2">INR {{ number_format((float) $this->paymentStats['online_amount'], 2) }}</flux:heading>
        </div>

        <div class="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900">
            <flux:text class="text-sm text-zinc-500">{{ __('Cash Paid') }}</flux:text>
    ...
    <div class="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
        <div>
            ...
            <flux:text class="mt-1 text-sm text-zinc-500">{{ __('Filter your payment records by status or search by library, seat, room, and reference.') }}</flux:text>
        </div>
        ...
        <div class="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <flux:input wire:model.live.debounce.300ms="search" label="Search" placeholder="Library, seat, room, order id, reference..." />
        </div>
    </div>
